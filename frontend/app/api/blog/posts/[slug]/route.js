...
export async function GET(request, { params }) {
  ...
  try {
    ...
    const post = await db.collection('blog_posts').findOne({ slug: params.slug, status: 'published' });
    ...
    db.collection('blog_posts').updateOne({ slug: params.slug }, { $inc: { viewCount: 1 } }).catch(() => {});
